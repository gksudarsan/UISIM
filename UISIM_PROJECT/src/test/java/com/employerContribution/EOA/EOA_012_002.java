...
public class EOA_012_002 extends TestBase {

	@Test(priority = 1, description = "EOA.012.002-Verify PEO MA can access the PEO account remove users.", groups = {
			"Regression" })
	public void EOA_012_002() throws Exception {
		commonStepDefinitions commonFuntions = new commonStepDefinitions();
		FIpage FI = new FIpage(driver);

		test = report.createTest("EOA.012.002-Verify PEO MA can access the PEO account remove users.");

		LoginPage loginPage = PageFactory.initElements(driver, LoginPage.class);
		commonFuntions.login(COMMON_CONSTANT.EMPLOYER_USER_2, COMMON_CONSTANT.EMPLOYER_USER_2_PASSWORD);
		...
		commonFuntions.enterTextboxContains("First Name", "David");
		...
		commonFuntions.screenShot("Employer Users", "Pass", "User Removed successfully");
		// Done
	}
}
