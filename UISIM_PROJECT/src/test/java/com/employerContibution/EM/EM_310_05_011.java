...
public class EM_310_05_011 extends TestBase{

	@Test
	public void EM_310_05_011() throws Exception
	{
		...
		test = 
				report.createTest("EM_310_05_011:Verify Employer is able to process Business Acquisition and indicate transfer 'Total' when a gap status exists  and system create task for CSR reviews.  ");
		...
		sleep();
		Map<String, String> databaseResults = cf.database_SelectQuery("SELECT * FROM T_EMPLOYER_ACCOUNT tea JOIN T_EMPLOYER_TRANSFER tr ON TEA.EMPLOYER_ACCOUNT_ID = tr.FROM_EMPLOYER_ID AND EAN IS NOT NULL ORDER BY tea.UPDATED_TS DESC");
		...
		//sleep(3000);
		empPage.commentBox_MyQ.sendKeys("Done");
