...
import com.ui.pages.SREG_027;
	...
	public class EM_475_006_Verify_CSR_is_able_to_enter_ERN_and_remove_POA_TPR_association_for_designation_type_Unemployment_Insurance_Administrative_Proceedings_and_Court_Appeals_Matters extends TestBase{
		...
		public void Testing123() throws Exception {
			...
			test = report.createTest("Verify_CSR_is_able_to_enter_ERN_and_remove_POA_TPR_association_for_designation_type_Unemployment_Insurance_Administrative_Proceedings_and_Court_Appeals_Matters");
			...
			Map<String, String> databaseEanResult =
					commonFunction.database_SelectQuerySingleColumn("SELECT * FROM T_EMPLOYER_ACCOUNT tea  WHERE EAN IS NOT NULL AND EAN IN (SELECT EAN FROM T_EMPLOYER te WHERE employer_id IN (SELECT EMPLOYER_ID FROM T_THIRD_PARTY_CDS_VENDOR_ASSOCIATION ttpcva WHERE DESIGNATION_TYPE = 'UIAPCAM' ))", "EAN");
			...
			commonFunction.selectDropdown("Designation Type", " Unemployment Insurance Administrative Proceedings and Court Appeals Matters ");
           // commonFunction.selectRadio(xpathParameter);
			commonFunction.selectRadio("Select");
	          ...
	          //commonFunction.enterTextbox("Association End Date", "05/17/2023");
			    PEOPage.calender.sendKeys("06/02/23");
