...
public class EM_443_008_Verify_CSR_Add_POA_Payroll_Agent_Agreement_plus extends TestBase {

	String EAN = prop.getProperty("EAN");

	@Test(priority = 1, description = "EM.443.008.Verify CSR is able to enter ERN and Add POA/TPR association for designation type Payroll Agent Agreement plus", groups = {
			"Regression" })
	public void EM_443_008() throws Exception {
		test = report.createTest(
				"EM.443.008.Verify CSR is able to enter ERN and Add POA/TPR association for designation type Payroll Agent Agreement plus");
...
//				"SELECT * FROM T_EMPLOYER_ACCOUNT tea2 WHERE ACCOUNT_STATUS = 'ACTV' AND EAN IS NOT NULL AND LENGTH(EAN)=7 ORDER BY UPDATED_TS",
		...
		sleep();
		cf.selectDropdownThirdParty(" Payroll Agent Agreement Plus - Solely for the purpose of filing returns and payments and responding to payroll agent inquires in order to resolve return filing issues. ");
