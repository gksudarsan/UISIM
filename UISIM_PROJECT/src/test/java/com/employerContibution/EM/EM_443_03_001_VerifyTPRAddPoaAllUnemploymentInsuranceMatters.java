...
public class EM_443_03_001_VerifyTPRAddPoaAllUnemploymentInsuranceMatters extends TestBase{

	@Test
	public void EM_443_03_001() throws Exception
	{
		...
		test = report.createTest("EM.443.03.001:Verify TPR is able to enter ERN and Add POA/TPR association for designation type \"All Unemployment insurance Matters\" and the system creates task for CSR reviews and approved.");
		cf.login(COMMON_CONSTANT.TPR_USER_2.toUpperCase(), COMMON_CONSTANT.TPR_USER_2_PASSWORD);
		...
		Map<String, String> databaseResults = 
				cf.database_SelectQuerySingleColumn("SELECT * FROM T_EMPLOYER_ACCOUNT tea WHERE ACCOUNT_STATUS = 'ACTV' AND EAN IS NOT NULL AND LENGTH(EAN)=7 ORDER BY UPDATED_TS", "EAN");
