...
public class EM_310_02_001_Verify_Employer_is_able_to_process_sale_of_business_and_indicate_transfer_total
	...
	@Test(priority = COMMON_CONSTANT.PRIORITY_1, description = "Verify EMP is able to process sale of business and indicate transfer 'total'", groups = {COMMON_CONSTANT.REGRESSION})
	public void Tc_310_02_001() throws Exception {
		...
		Map<String, String> dataBaseResults1 = commonFuntions.database_SelectQuery("SELECT * FROM T_EMPLOYER_ACCOUNT tea WHERE ean IN (SELECT EAN FROM T_EMPLOYER te WHERE EMPLOYER_ID IN (SELECT TO_EMPLOYER_ID FROM T_EMPLOYER_TRANSFER_SUCCESSOR WHERE NOT_100_PERCENT_TRANSFER_EXPLAIN = 'Total Transfer'))"); 
	...
	Map<String, String> databaseResults = commonFuntions.database_SelectQuerySingleColumn(
			"SELECT * FROM T_EMPLOYER_ACCOUNT tea WHERE ean IN (SELECT EAN FROM T_EMPLOYER te WHERE EMPLOYER_ID IN (SELECT TO_EMPLOYER_ID FROM T_EMPLOYER_TRANSFER_SUCCESSOR WHERE NOT_100_PERCENT_TRANSFER_EXPLAIN = 'Total Transfer'))", "ENTITY_NAME");
