...
public class EM_475_008_Verify_CSR_is_able_to_enter_ERN_and_remove_POA_TPR_association_for_designation_type_Payroll_Agent_Agreement_plus extends TestBase{
	...
	public void Testing123() throws Exception {
		...
		test = report.createTest("EM_475_008_Verify_CSR_is_able_to_enter_ERN_and_remove_POA_TPR_association_for_designation_type_Payroll_Agent_Agreement_pluss");
		...
		Map<String, String> databaseEanResult =
				commonFunction.database_SelectQuerySingleColumn("SELECT * FROM T_EMPLOYER_ACCOUNT tea  WHERE EAN IS NOT NULL AND EAN IN (SELECT EAN FROM T_EMPLOYER te WHERE employer_id IN (SELECT EMPLOYER_ID FROM T_THIRD_PARTY_CDS_VENDOR_ASSOCIATION ttpcva WHERE DESIGNATION_TYPE = 'PAAP' ))", "EAN");
        ...
        sleep(1000);
        commonFunction.ScrollMenu("Designation Type");
        commonFunction.selectDropdown("Designation Type", " Payroll Agent Agreement Plus - Solely for the purpose of filing returns and payments and responding to payroll agent inquires in order to resolve return filing issues. ");
        sleep(1000);
...
//commonFunction.enterTextbox("Association End Date", "05/17/2023");
        ...
        sleep(1000);
        commonFunction.screenShot("deatils entered", "Pass", "Details entered sucessfuly");
