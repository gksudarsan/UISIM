...
public class EM_475_02_008 extends TestBase
{


	
	@Test(priority=1, description = "EM.475.02.008 - Verify Employer is able to remove POA/TPR association for designation type \"Payroll Agent Agreement plus\".",groups = {"Regression"})
	public void EM_475_02_008() throws Exception
	{
		 
		 test = report.createTest("EM.475.02.008 - Verify Employer is able to remove POA/TPR association for designation type \"Payroll Agent Agreement plus\".");
		 commonStepDefinitions commonFuntions = new commonStepDefinitions();
		 Map<String, String> databaseEanResult = commonFuntions.database_SelectQuerySingleColumn("SELECT * FROM T_EMPLOYER_ACCOUNT tea  WHERE EAN IS NOT NULL AND EAN IN (SELECT EAN FROM T_EMPLOYER tea WHERE employer_id IN (SELECT EMPLOYER_ID FROM T_THIRD_PARTY_CDS_VENDOR_ASSOCIATION ttpcva WHERE DESIGNATION_TYPE = 'PAAP'))","EAN");
