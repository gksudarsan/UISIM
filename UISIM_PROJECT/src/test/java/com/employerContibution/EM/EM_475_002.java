...
public class EM_475_002 extends TestBase
{


	
	@Test(priority=1, description = "EM.475.002- Verify CSR is able to search POA/TPR detail and remove the association for POA/TPR with designation type in \"Limited Unemployment Insurance Matters\".",groups = {"Regression"})
	public void EM_475_002() throws Exception
	{
		 
		 test = report.createTest("EM.475.002- Verify CSR is able to search POA/TPR detail and remove the association for POA/TPR with designation type in \"Limited Unemployment Insurance Matters\".");
		 commonStepDefinitions commonFuntions = new commonStepDefinitions();
		 Map<String, String> databaseEanResult = commonFuntions.database_SelectQuerySingleColumn("SELECT * FROM T_EMPLOYER_ACCOUNT tea  WHERE EAN IS NOT NULL AND EAN IN (SELECT EAN FROM T_EMPLOYER te WHERE employer_id IN (SELECT EMPLOYER_ID FROM T_THIRD_PARTY_CDS_VENDOR_ASSOCIATION ttpcva WHERE designation_type='LUIM'))","EAN");
