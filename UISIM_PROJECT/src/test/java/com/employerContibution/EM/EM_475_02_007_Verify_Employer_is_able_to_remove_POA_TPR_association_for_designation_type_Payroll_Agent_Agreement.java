package com.employerContibution.EM;
	...
	import com.ui.pages.SREG_027;
		...
		public class EM_475_02_007_Verify_Employer_is_able_to_remove_POA_TPR_association_for_designation_type_Payroll_Agent_Agreement extends TestBase{
			...
			public void Testing123() throws Exception {
				...
				test = report.createTest("EM_475_02_007_Verify_Employer_is_able_to_remove_POA_TPR_association_for_designation_type_Payroll_Agent_Agreement");
				...
				Map<String, String> databaseEanResult =
						commonFunction.database_SelectQuerySingleColumn("SELECT * FROM T_EMPLOYER_ACCOUNT tea  WHERE EAN IS NOT NULL AND EAN IN (SELECT EAN FROM T_EMPLOYER te WHERE employer_id IN (SELECT EMPLOYER_ID FROM T_THIRD_PARTY_CDS_VENDOR_ASSOCIATION ttpcva WHERE DESIGNATION_TYPE = 'PAA' ))", "EAN");
				...
				commonFunction.login(COMMON_CONSTANT.EMP_USER_1.toUpperCase(), COMMON_CONSTANT.EMP_USER_1_PASSWORD);
				sleep(2000);
		 ...
		 * eanValue); commonFunction.clickButton("Continue ");
		 */  sleep(1000);
         commonFunction.screenShot("SREG-537", "Pass", "Add or Remove Third Party Association to Employer page is displayed");
		   commonFunction.selectDropdown("Designation Type", "Payroll Agent Agreement - Solely for the purpose of filing returns and payments");
        sleep(1000);
       ...
       //commonFunction.enterTextbox("Association End Date", "05/17/2023");
		    //PEOPage.calender.sendKeys("05/17/23");
