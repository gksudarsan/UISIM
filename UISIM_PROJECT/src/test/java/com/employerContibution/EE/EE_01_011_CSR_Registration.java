...
public class EE_01_011_CSR_Registration extends TestBase{
	...
	public void EE_01_011() throws Exception
	{
		...
		Map<String, String> databaseResults = commonFuntions.database_SelectQuerySingleColumn("SELECT * FROM T_EMPLOYER_ACCOUNT tea WHERE FEIN NOT IN (SELECT FEIN FROM T_EMPLOYER_DOL_DTF tedd) ORDER BY UPDATED_TS DESC", "FEIN");
		...
		Map<String, String> databaseResults1 = commonFuntions.database_SelectQuerySingleColumn("SELECT EAN FROM T_EMPLOYER_ACCOUNT tea EXCEPT SELECT ERN  FROM T_EMPLOYER_DOL_DTF tedd ", "EAN");
		...
		commonFuntions.enterTextboxContains("Federal Employer Identification Number (FEIN)", FEIN1);sleep();
		...
		AddPage.clickOnLink(" Add Another Acquisition");	
		commonFuntions.selectRadioQuestions("Have you acquired the business of another employer liable for New York State Unemployment Insurance?", "Yes");
		Map<String, String> databaseResults4 = commonFuntions.database_SelectQuerySingleColumn("SELECT * FROM T_EMPLOYER_ACCOUNT tea", "FEIN");
		...
		sleep(2000);
		Map<String, String> databaseResults5 = commonFuntions.database_SelectQuerySingleColumn("SELECT * FROM T_EMPLOYER_ACCOUNT tea WHERE EAN IS NOT NULL ORDER BY UPDATED_TS DESC", "EAN");
		...
		commonFuntions.enterTextboxContains("SSN", "345646546");
