...
public class EE_11_004_TPR_UserType extends TestBase {
	...
	@Test
	public void EE_11_004_TPR() throws Exception {
		commonStepDefinitions commonFuntions = new commonStepDefinitions();
		PEOPage PEOPage = PageFactory.initElements(driver, PEOPage.class);
		AddressPage AddPage = PageFactory.initElements(driver, AddressPage.class);

		test = report.createTest(
				"EE.11.004:Verify TPR can submit employer registration for employer type 'Non-Profit' and legal entity type 'Joint Venture' and work items will be created for CSR to review.");
		...
		// SELECT * FROM T_EMPLOYER_ACCOUNT tea WHERE FEIN IN (SELECT FEIN FROM
		// T_EMPLOYER_DOL_DTF tedd) ORDER BY UPDATED_BY DESC
		Map<String, String> databaseResults = commonFuntions.database_SelectQuerySingleColumn(
				"SELECT * FROM T_EMPLOYER_ACCOUNT tea ORDER BY UPDATED_BY DESC", "FEIN");
		...
		Map<String, String> databaseResults1 = commonFuntions.database_SelectQuerySingleColumn(
				"SELECT * FROM T_EMPLOYER_ACCOUNT tea ORDER BY UPDATED_BY DESC", "EAN");
		...
		commonFuntions.enterTextboxContains("Business Email Address",
				"autoTest" + Long.toString(commonFuntions.createRandomInteger(10000, 99999)) + "@gmail.com");
		commonFuntions.enterPastDate("Enter date of first operations", 540);
		...
		commonFuntions.enterTextbox("Program Name", "automation testing program");
		commonFuntions.selectDropdown("Principle purpose for which you are organized and operate.", " Nursing Home ");
		commonFuntions.screenShot("AddPrimaryBusinessPhysicalAddress", "Pass",
				"Add Primary Business Physical Address:SREG-008");
		commonFuntions.clickButtonContains("Continue");
		sleep();
		commonFuntions.waitForLoadingIconToDisappear();
		...
		commonFuntions.screenShot("BusinessPhysicalAddressDetails", "Pass",
				"Bussiness Physical Address Details:SREG-007");
		commonFuntions.clickButtonContains("Continue");
		sleep();
		commonFuntions.waitForLoadingIconToDisappear();
		...
		commonFuntions.screenShot("AddCorporateOfficerDetails", "Pass",
		...
		/*---Statement of Acknowledgement---SREG-043*/
		...
		// UPDATE LROUIM.T_WFA_WORK_ITEM_DETAIL SET USER_ID = 'registrationspecialist'
		// AND WORK_ITEM_STATUS_CODE = 'RESV' AND UPDATED_BY = 'NDJSK4' AND UPDATED_TS =
		// CURRENT_TIMESTAMP WHERE WORK_ITEM_DETAIL_ID = '128066' AND WORK_ITEM_NAME =
		// 'ReviewEmployerType' AND CREATED_BY = 'pstpruser';
		commonFuntions.logoutAndLogin(COMMON_CONSTANT.CSR_USER_9.toUpperCase(), COMMON_CONSTANT.CSR_USER_9_PASSWORD);
		sleep(10000);

		// Resolving WI Review employer type................
		PEOPage.queue.click();
		sleep();commonFuntions.waitForLoadingIconToDisappear();
		commonFuntions.screenShot("individualWorkQueue", "Pass", "Individual Work Queue");
		...
		sleep();commonFuntions.waitForLoadingIconToDisappear();
		commonFuntions.screenShot("ReviewEmployerTypeTask", "Pass", "Review Employer Type Task");
		AddPage.commentField.sendKeys("closing review employer type task");
		commonFuntions.clickButtonContains("Submit");
		sleep();commonFuntions.waitForLoadingIconToDisappear();
