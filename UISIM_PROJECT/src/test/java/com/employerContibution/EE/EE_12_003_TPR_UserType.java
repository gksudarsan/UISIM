...
public class EE_12_003_TPR_UserType extends TestBase{
	...
	public void EE_12_003() throws Exception
	{
		...
		sleep(3000);
		commonFuntions.enterTextboxContains("First Name", "ThirdPartyTest");
		commonFuntions.enterTextboxContains("Last Name", "ThirdParty");
		...
		Map<String, String> databaseResults = commonFuntions.database_SelectQuerySingleColumn("SELECT * FROM T_EMPLOYER_ACCOUNT tea WHERE FEIN NOT IN (SELECT FEIN FROM T_EMPLOYER_DOL_DTF tedd) ORDER BY UPDATED_TS DESC", 
				"FEIN");
		...
		commonFuntions.clickButtonContains("Continue");
		sleep(3000);
		//AddPage.legalNameTextBox.sendKeys("TESTING FRAMEWORK LLC");
		...
		commonFuntions.enterTextboxContains("Zip Code", commonFuntions.createRandomInteger(100, 999)+"46");
		...
		AddPage.lastName_locationOfBooksAndrecords.sendKeys("Beats");
		sleep();
	    ...
	    commonFuntions.clickButtonContains("Open Work Item");
	    sleep(2000);
	    commonFuntions.enterPastDate("Liability Date", 30);
	    AddPage.commentField.sendKeys("Test1");
	    commonFuntions.clickButtonContains("Submit");
	    sleep(3000);
	    commonFuntions.screenShot("ObtainBondTaskTypeCreatedSuccessfully", "Pass", "WorkItem Completed:SUC-002:Obtain Bond Task");
	    sleep(2000);
	    commonFuntions.clickButtonContains("Home");
	    sleep(2000);
	    
	    //Assigning USER IO to WI
	    commonFuntions.database_UpdateQuery("UPDATE LROUIM.T_WFA_WORK_ITEM_DETAIL SET USER_ID = '"+COMMON_CONSTANT.CSR_USER_1+"' WHERE PROCESS_DETAIL_ID IN (SELECT PROCESS_DETAIL_ID FROM T_WFA_PROCESS_DETAIL WHERE FEIN='"+feinValue+"' ORDER BY UPDATED_TS desc)");
	    sleep(2000);
	    PEOPage.queue.click();
		sleep(15000);
