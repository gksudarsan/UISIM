...
public class BCL_855_08_003 extends TestBase {

	@Test(priority = 1, description = "BCL.855.08.003 -   Verify CSR can search ERN details and add new prosecution for Prosecution Type is  'Task Force' and status is 'Plea Agreement'", groups = {
			"Regression" })
	public void BCL_855_08_003() throws Exception {
		commonStepDefinitions commonFuntions = new commonStepDefinitions();
		PEOPage PEOPage = PageFactory.initElements(driver, PEOPage.class);
		BclPage BCL = new BclPage(driver);

		Map<String, String> databaseEanResult = commonFuntions
				.database_SelectQuerySingleColumn("SELECT * FROM T_EMPLOYER_ACCOUNT tea", "EAN");
		String eanValue = databaseEanResult.get("EAN");
		System.out.println(eanValue);

		test = report.createTest(
				"BCL.855.08.003 -   Verify CSR can search ERN details and add new prosecution for Prosecution Type is  'Task Force' and status is 'Plea Agreement'");
		...
		commonFuntions.screenShot("Enter Prosecution Details", "Pass",
				"Enter Prosecution Details (COL-595)screen launched");
		...
		commonFuntions.screenShot("Enter Prosecution Details", "Pass",
				"Entered Prosecution Details on (COL-595)screen");
		commonFuntions.clickButtonContains("Submit ");
		sleep(2000);

		// ---SUC-002---
		commonFuntions.screenShot("Prosecution Confirmation", "Pass",
				"Prosecution Confirmation (SUC-002)screen launched");
