...
public class BCL_802_05_002 extends TestBase {

	@Test(priority=1, description = "BCL.802.05.002 -  Verify CSR can Search FEIN with select bankruptcy case status 'Jointly Administered' and add Bankruptcy Case Activity for activity type incoming and activity name 'Bankruptcy Petition'",groups = {"Regression"})
	public void BCL_802_05_002() throws Exception
	{
		commonStepDefinitions commonFuntions= new commonStepDefinitions();
		PEOPage PEOPage = PageFactory.initElements(driver, PEOPage.class);
		BclPage BCL = new BclPage(driver);
		
        Map<String, String> databaseEanResult = commonFuntions.database_SelectQuerySingleColumn("SELECT * FROM T_TX_BANKRUPTCY tea","CASE_NUMBER");
        String CASE_NUMBER = databaseEanResult.get("CASE_NUMBER");
        System.out.println(CASE_NUMBER);
        
		test = report.createTest("BCL.802.05.002 -  Verify CSR can Search FEIN with select bankruptcy case status 'Jointly Administered' and add Bankruptcy Case Activity for activity type incoming and activity name 'Bankruptcy Petition'");
		...
		//---COL-474---
		commonFuntions.screenShot("Review/Update Bankruptcy Case Activity", "Pass", "Review/Update Bankruptcy Case Activity (COL-474)screen launched");
		commonFuntions.enterTextboxContains("a. Bankruptcy Case Number", CASE_NUMBER);
		commonFuntions.selectDropdown("2. Bankruptcy Status", " Jointly Administered ");
		...
		commonFuntions.selectDropdown("3. Activity Name", " Bankruptcy Petition ");
