...
public class BCL_855_08_008_CSRsearchERN_UpdateExistingProsecution_TypeCivil_statusClosed extends TestBase {

	@Test(priority = COMMON_CONSTANT.PRIORITY_1, description = "Verify CSR can search ERN details and update existing prosecution for Prosecution Type is 'Civil' and status is 'Closed' ", groups = "Regression")
	public void BCL_855_08_008() throws Exception {

		test = report.createTest("BCL_855_08_008_Verify CSR can search ERN details and update existing prosecution for Prosecution Type is 'Civil' and status is 'Closed' ");
		
		commonStepDefinitions commonFuntions = new commonStepDefinitions();
		//BclPage bcllocators = new BclPage(driver);
		
		Map<String, String> databaseEanResult = commonFuntions.database_SelectQuerySingleColumn("SELECT * FROM T_employer WHERE EMPLOYER_ID in(SELECT EMPLOYER_ID FROM T_TX_PROSECUTION WHERE PROSECUTION_STATUS_CD = 'ACTV') AND EAN IS NOT NULL AND LENGTH (EAN)=7", "EAN");
