...
public class BCL_802_05_007_CSR_SearchBankruptcyErn_Terminated_AddBankruptcyCase_ClaimWithdrawalAcknowledgement
		extends TestBase {

	@Test(priority = COMMON_CONSTANT.PRIORITY_1, description = "Verify CSR can Search ERN with select bankruptcy case status 'Terminated' and add Bankruptcy Case Activity for activity type incoming and activity name 'Claim Withdrawal Acknowledgement'", groups = {
			COMMON_CONSTANT.REGRESSION })
	public void TC_BCL_802_05_007() throws Exception {

		test = report.createTest(
				"BCL.802.05.007 : Verify CSR can Search ERN with select bankruptcy case status 'Terminated' and add Bankruptcy Case Activity for activity type incoming and activity name 'Claim Withdrawal Acknowledgement'");
		...
		// --- Login ---
		// commonFunction.login(COMMON_CONSTANT.CSR_USER_1.toUpperCase(),
		// COMMON_CONSTANT.CSR_USER_1_PASSWORD);
		commonFunction.login(COMMON_CONSTANT.Collections_Specialist_3_User,
				COMMON_CONSTANT.Collections_Specialist_3_pwd);
		...
		commonFunction.screenShot("MenuPage", "Pass", "Menu page apeared");
		...
		commonFunction.screenShot("MenuPage", "Pass",
				"Navigate to Menu -> Contribution Collection -> Bankruptcy -> Review/Update Bankruptcy Case Activity");
		commonFunction.clickMenu("Review/Update Bankruptcy Case Activity");

		// --- COL-474 ---
		commonFunction.waitForLoadingIconToDisappear();
		commonFunction.screenShot("BCL80205007", "Pass",
				"Successfully launched to Review/Update Bankruptcy Case Activity(COL-474) screen");
		...
		sleep();
		commonFunction.selectRadioInTable("04-64364", 1, 1, "");
		sleep(2000);
		...
		commonFunction.waitForLoadingIconToDisappear();
		commonFunction.screenShot("BCL80205007", "Pass",
				"Successfully launched to View Bankruptcy Case Deatils/History(COL-475) screen");
		commonFunction.clickButtonContains("Add New Activity ");

		// --- COL-402 ---
		commonFunction.waitForLoadingIconToDisappear();
		commonFunction.screenShot("BCL80205007", "Pass",
				"Successfully launched to Add/Edit Activity - Bankruptcy Case Activity(COL-402) screen");
		...
		commonFunction.waitForLoadingIconToDisappear();
		commonFunction.screenShot("BCL80205007", "Pass",
				"Successfully launched to Add/Edit - Activity Verification(COL-513) screen");
