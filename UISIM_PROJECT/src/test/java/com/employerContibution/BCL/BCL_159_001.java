...
public class BCL_159_001 extends TestBase {
	...
	public void BCL_159_001() throws Exception {
		...
		test = report.createTest(
				"BCL.159.001-Verify CSR can submit collection notice for single quarter and notice type is Enforcement Letter");
		//cf.login(COMMON_CONSTANT.CSR_USER_1.toUpperCase(), COMMON_CONSTANT.CSR_USER_1_PASSWORD);
		cf.login(COMMON_CONSTANT.Collections_Specialist_1.toUpperCase(), COMMON_CONSTANT.Collections_Specialist_1_PASSWORD);
		...
		cf.waitForLoadingIconToDisappear();//new added due app slowness
		cf.errorContent("The Employer Registration Number is invalid.");
		cf.enterTextboxContains("Employer Registration Number", "1023453");
		cf.waitForLoadingIconToDisappear();//new added due app slowness
		cf.clickButtonContains("Continue ");
		cf.waitForLoadingIconToDisappear();
		cf.screenShot("GenerateCollectionNoticeEnterERN4", "Pass",
				"Collection Notice cannot be issued as there is no debt on the account.");
		sleep();
		cf.waitForLoadingIconToDisappear();//new added due app slowness
		...
		cf.waitForLoadingIconToDisappear();//new added due app slowness
		cf.enterCurrentDate("Notice Date");
		sleep();
